...
│   ├── inspector.js        ← unified inspector panel (event/note editing, schedule setup, settings modal, toolbar wiring)
...
1. **Foundation:** constants.js (in `<head>`) → app-state.js (Store) → utils.js → ui-core.js
2. **Data layer:** schema.js → data-helpers.js → persistence.js
3. **Rendering:** render.js → print.js
4. **UI:** events.js → inspector.js (unified panel: event/note editing, schedule setup, settings modal, toolbar)
